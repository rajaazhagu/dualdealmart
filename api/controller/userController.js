...
const payMonth =(async(req,res)=>{
  const {paymentId,email,date,month,count,year} =req.body
  try {
    const data = await User.findOne({email:email})
    if(data){
      await User.updateOne({email:email},{$set:{TransactionId:paymentId,date:date,month:month,count:count,year:year}})
      res.json('paid')
    }
  } catch (error) {
    console.log(error)
